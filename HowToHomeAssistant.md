...
KNXnet/IP device management, the stick takes them from the top of its own
line: `<area>.<line>.241` to `<area>.<line>.250` — **`15.15.241` to `15.15.250`**
while the stick still has the default address `15.15.0`. If the stick's own
address lies in that range, it is skipped and the range ends at `.251`. Firmware
up to v1.4.29 used `.1` to `.10`; a stick updated from it moves to the new range
